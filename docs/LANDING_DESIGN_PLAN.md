...
| CF1 | **Deploy**: build + `wrangler pages deploy` ke project `socio-id` — pakai skill `cloudflare`/`wrangler`; cek header keamanan + cache rules | ½ sesi | ✅ **SELESAI 2 Sep (CUTOVER LIVE)** — Dua tahap: (1) Deploy awal ke akun salah (3smedianet `766dfffa…` → project duplikat `socio-id-er2.pages.dev`, preview only). (2) **Cutover production**: user kasih akun benar (socio.id `0298214d…`, zone `22e8176e…`, token di `accountcf.md` gitignored). Ternyata project `socio-id` di akun benar SUDAH ADA (17 Jul, landing lama `feea964a`) + domain `socio.id`/`www` ter-attach + DNS apex/www sudah CNAME `socio-id.pages.dev` proxied — cukup deploy build baru. Deploy `9c8e6686` (70 file, 2 Sep 02:35 UTC) → **socio.id LIVE landing baru tanpa ubah DNS**. Verified: homepage 93KB (lama 159KB), title baru, CSP/HSTS/X-Frame live, 3 artikel blog 200, sitemap 200 host `https://socio.id`, llms.txt 200, cache `/_astro/*` immutable 1y live, www serve konten sama. DNS lain (app/cdn/coolify/mail/MX/TXT) TIDAK disentuh — snapshot rollback `/tmp/dns-rollback-socio-id.txt`. **Runbook**: `docs/LANDING_DEPLOY.md` (deploy manual, verifikasi, rollback, riwayat cutover). **Catatan**: `app.socio.id` 522 = VPS down pre-existing (bukan akibat cutover, DNS app utuh) — user akan kasih akses VPS untuk setup Coolify+DB+app (M1/M2 infra). |
