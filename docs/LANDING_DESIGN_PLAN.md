...
| CF1 | **Deploy**: build + `wrangler pages deploy` ke project `socio-id` — pakai skill `cloudflare`/`wrangler`; cek header keamanan + cache rules | ½ sesi | ✅ **SELESAI 2 Sep** — Project `socio-id` **dibuat baru** via `wrangler pages project create` (plan sebelumnya bilang project sudah ada — ternyata tidak; OAuth wrangler 3smedianet@gmail.com account `766dfffa…` punya Pages write). Deploy 2× (70 file): `https://socio-id-er2.pages.dev` (production alias) + deployment `5a26c005`. **Security headers via `landing/public/_headers`** (baru): HSTS preload 1y, X-Frame-Options DENY, X-Content-Type-Options nosniff, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy (camera/mic/geo/payment/usb/interest-cohort all denied), CSP `default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; frame-ancestors 'none'; form-action 'self' https://app.socio.id; object-src 'none'` (inline diperlukan: JSON-LD + module script + style attr; semua aset self-host, 0 external asset origin). **Cache rule `/_astro/*` immutable 1y**. Verified live: home 200, `/blog/smm-panel/` 200, `/blog/smm-panel-indonesia/` 200, sitemap-index → sitemap-0 (19 URL, host `https://socio.id`), robots.txt AI allowlist, llms.txt ada. **Custom domain `socio.id` BELUM di-attach** — masih situs PHP lama live via CF proxy; cutover domain butuh approval user terpisah (M7). |
