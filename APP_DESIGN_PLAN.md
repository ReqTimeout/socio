...
**DILARANG**: React/Vue/jQuery/Bootstrap, WebSocket (pakai SSE), Redis, illustration flat generic.
...
## 3. App Shell — Responsive (Mobile bottom-nav + Desktop sidebar)

### Mobile (`< lg`, 1024px) — bottom-nav shell
...
│  Nav:    │   - Multi-column grids           │
│  Beranda │   - Dense tables                 │
│  Layanan │   - Inline stats                 │
│  Pesanan │                                  │
│  Saldo   │                                  │
│  Akun    │                                  │
│  ──────  │                                  │
...
- **Sidebar** (`Sidebar.svelte`): `hidden lg:flex`, fixed left, `w-64`, `border-r`.
- **BottomNav**: `lg:hidden` (mobile only).
- **FAB**: `lg:hidden` (mobile only — sidebar covers nav on desktop).
- **Main**: `flex-1`, content `max-w-7xl mx-auto px-6 lg:px-8`.
...
6. **Jangan lupa desktop layout** — user app wajib responsive: mobile bottom-nav + desktop sidebar (Linear/Stripe style, `lg` breakpoint).
...
export const actions: Actions = {
  create: async ({ request, locals }) => {
    ...
    if (method === "midtrans") {
      const snapUrl = await createMidtransSnap(
        invoiceId,
        postAmount,
        locals.user!,
      );
...
const quantity = isCustomComments
  ? komen.split("\n").filter(Boolean).length
  : Number(form.get("quantity"));

if (isCustomComments && !komen)
  return fail(400, {
    error: "Komen wajib diisi untuk layanan Custom Comments",
  });
if (!isCustomComments && (!quantity || quantity < s.min))
  return fail(400, { error: `Min ${s.min}` });
...
11. **G-U13: Halaman notif** — `/notif` list + mark as read.
...
_Dokumen ini kontrak untuk frontend agent. Kalau kontradiksi dengan `REBUILD_PLAN.md`, REBUILD_PLAN menang. Kalau ragu, tanya user._
