...
<section class="space-y-4">
  ...
  <div class="grid grid-cols-2 gap-2">
    {#each chips as c}
      <button onclick={() => setChip(c)} class="rounded-2xl border p-4 text-center font-semibold {amount === c && !custom ? 'border-accent-ink bg-accent-50 text-accent-700' : 'border-ink-200'}">
        {formatRupiah(c)}
      </button>
    {/each}
  </div>
  ...
  <form
    method="POST"
    action="?/topup"
    use:enhance={() => { submitting = true; return async ({ result }) => { submitting = false; if (result.type === "failure") toast((result.data as any)?.error ?? "Gagal", "error"); else { toast("Permintaan top up dibuat", "success"); await applyAction(result); } }; }}
  >
  ...
  {#if !data.midtransReady && !data.tripayReady}
    <p class="text-center text-xs text-ink-400">Pembayaran otomatis (Midtrans/Tripay) akan aktif setelah konfigurasi payment gateway.</p>
  {/if}
    ...
    {:else}
      <ul class="space-y-2">
        {#each data.history as h (h.id)}
          <li class="flex items-center justify-between rounded-xl border border-ink-100 px-4 py-3 text-sm">
            <div>
              <div class="font-semibold">{formatRupiah(Number(h.amount))}</div>
              <div class="text-xs text-ink-500">{h.methodName}</div>
            </div>
            <span class="rounded-full px-2 py-0.5 text-xs font-semibold {h.status === 'Success' ? 'bg-success/10 text-success' : h.status === 'Canceled' ? 'bg-danger/10 text-danger' : 'bg-status-pending/10 text-status-pending'}">{h.status}</span>
          </li>
        {/each}
      </ul>
    {/if}
