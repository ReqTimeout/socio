...
<section class="space-y-3">
  <!-- Search -->
  <form onsubmit={(e) => { e.preventDefault(); onSearch(); }} class="sticky top-14 z-30 -mx-4 border-b border-ink-100 bg-surface/95 px-4 py-2 backdrop-blur">
    <div class="flex gap-2">
      ...
      />
      <button type="submit" class="grid h-10 w-10 place-items-center rounded-xl bg-accent-ink text-white" aria-label="Cari">
  ...
  <div class="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
    <button onclick={() => selectCat(0)} class="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold {!data.params.cat ? 'bg-ink-900 text-white' : 'bg-ink-100 text-ink-700'}">Semua</button>
    {#each data.categories as c}
      <button onclick={() => selectCat(c.id)} class="shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold {Number(data.params.cat) === c.id ? 'bg-ink-900 text-white' : 'bg-ink-100 text-ink-700'}">{c.name}</button>
    {/each}
  </div>
  ...
  <div class="flex items-center justify-between text-xs text-ink-500">
    ...
    <div class="flex gap-1">
      {#each [["termurah","Termurah"],["termahal","Termahal"],["terlaris","Terlaris"]] as [val, label]}
        <button onclick={() => selectSort(val)} class="rounded-full px-2 py-1 font-semibold {data.params.sort === val ? 'text-accent-ink' : ''}">{label}</button>
      {/each}
    </div>
  </div>
  ...
  {#if data.services.length === 0}
    <EmptyState title="Layanan tidak ditemukan" description="Coba kata kunci lain atau ganti kategori." />
  {:else}
    <ul class="space-y-2">
    ...
    {#if data.hasMore}
      <button onclick={loadMore} disabled={pending} class="w-full rounded-xl border border-ink-200 py-3 text-sm font-semibold text-ink-600 hover:bg-ink-50">
