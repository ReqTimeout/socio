<script lang="ts">
  ...
  async function share() {
    haptic();
    if (navigator.share) {
      await navigator.share({ title: "Socio.id", text: "Bergabung lewat referral saya", url: data.refLink });
    } else {
...
<section class="space-y-4">
  ...
  <div class="rounded-2xl bg-ink-900 p-4 text-white">
    ...
    <div class="font-display text-2xl font-extrabold tabular-nums">{formatRupiah(data.commission)}</div>
  ...
  <div class="rounded-2xl border border-ink-100 bg-surface p-4">
    ...
    <div class="flex gap-2">
      <input readonly value={data.refLink} class="h-10 flex-1 rounded-xl border border-ink-200 bg-ink-50 px-3 text-sm" />
