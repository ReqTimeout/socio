...
>
  <div class="flex-1 flex flex-col justify-center max-w-sm w-full mx-auto">
    <div class="mb-8 text-center">
      ...
      <p class="text-ink-500 mt-2 text-sm">Reset password akun Anda</p>
    </div>

    {#if form?.success}
      <div class="rounded-2xl bg-success-soft text-success text-sm px-4 py-3 font-medium">
        Jika email terdaftar, link reset sudah dikirim. Cek kotak masuk Anda.
      </div>
      <div class="mt-6 text-center">
        <a href="/login" class="text-sm font-semibold text-primary">Kembali ke masuk</a>
      </div>
    {:else}
      {#if form?.error}
        <div
      ...
      >
        ...
        <Button type="submit" full disabled={loading} onclick={() => (loading = true)}
          >{loading ? "Memproses…" : "Kirim link reset"}</Button
        >
      </form>
